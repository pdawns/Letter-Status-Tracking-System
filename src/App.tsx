...
function App() {
  ...
  const handleQRScanSuccess = (decodedText: string) => {
    ...
    // Extract track param using regex — handles all URL formats including localhost:5173/?track=uuid
    const trackMatch = text.match(/[?&]track=([a-f0-9-]{36})/i);
    if (trackMatch) {
      setCurrentLetterId(trackMatch[1]);
      setView('track');
      return;
    }
    ...
    if (uuidRegex.test(text)) {
      setCurrentLetterId(text);
      setView('track');
      return;
    }

    alert('Invalid QR code. This QR code is not from this system.');
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
              ...
              >
                <div className="flex flex-col items-center text-center space-y-3">
                  ...
                  <p className="text-gray-600 text-sm">Scan via camera or upload a QR code image/PDF</p>
                </div>
              </button>
