...
export default function HandlerUpdate({ letterId, onBack }: HandlerUpdateProps) {
    ...
    <div className="flex items-center justify-center p-6">
      <div className="rounded-2xl p-6 max-w-md w-full" style={{ background: 'var(--card-bg)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid rgba(var(--accent-rgb),0.2)', boxShadow: '0 4px 24px rgba(0,0,0,0.3)' }}>
        ...
        <form onSubmit={handlePinSubmit} className="space-y-3">
          <div>
            <label htmlFor="pin" className="block text-xs font-medium mb-1" style={{ color: 'rgba(var(--accent-rgb),0.85)' }}>PIN</label>
  ...
  return (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--card-bg)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(var(--accent-rgb),0.2)', boxShadow: '0 4px 24px var(--input-bg)' }}>
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="rounded-2xl overflow-hidden" style={{
              ...
              border: isReviewed ? '1px solid rgba(16,185,129,0.25)' : '1px solid rgba(var(--accent-rgb),0.15)',
            }}>
              {/* Step 1 header row */}
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(var(--accent-rgb),0.1)' }}>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                    style={{ background: isReviewed ? '#10b981' : 'var(--primary)' }}>
                    {isReviewed ? '✓' : '1'}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: 'var(--accent-text)' }}>For Review</p>
                    <p className="text-xs" style={{ color: 'rgba(var(--accent-rgb),0.7)' }}>{SIR_LENMARK}</p>
                  </div>
                </div>
                {isReviewed
              ...
              <div className="px-4 py-3">
                {isReviewed && reviewStatus ? (
                  <div className="text-xs space-y-0.5" style={{ color: 'rgba(var(--accent-text-rgb),0.75)' }}>
                    <p>Reviewed by: <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{reviewStatus.signed_by}</span></p>
                    <p style={{ color: 'rgba(var(--accent-rgb),0.65)' }}>Date: {new Date(reviewStatus.signed_at).toLocaleString()}</p>
                    ...
                    {reviewChecked && (
                      <>
                        ...
                        <button onClick={() => setShowReviewConfirm(true)} disabled={saving}
                          ...
                          style={{ backgroundColor: 'var(--primary)' }}
                          onMouseEnter={(e) => !saving && (e.currentTarget.style.backgroundColor = 'var(--primary-hover)')}
                          onMouseLeave={(e) => !saving && (e.currentTarget.style.backgroundColor = 'var(--primary)')}>
                          Save Review
                        </button>
                      </>
                    )}
            ...
            <div className="rounded-2xl overflow-hidden" style={{
              ...
              border: !isReviewed ? '1px solid rgba(var(--accent-rgb),0.15)' : isApproved ? '1px solid rgba(16,185,129,0.25)' : '1px solid rgba(var(--accent-rgb),0.15)',
              ...
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(var(--accent-rgb),0.1)' }}>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                    style={{ background: isApproved ? '#10b981' : isReviewed ? 'var(--primary)' : '#4b5563' }}>
                    {isApproved ? '✓' : '2'}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: 'var(--accent-text)' }}>For Approval</p>
                    <p className="text-xs" style={{ color: 'rgba(var(--accent-rgb),0.7)' }}>{SIR_RONALD}</p>
                  </div>
                </div>
                {!isReviewed
              ...
              <div className="px-4 py-3">
                {isApproved && approvalStatus ? (
                  <div className="space-y-1">
                    <div className="text-xs space-y-0.5" style={{ color: 'rgba(var(--accent-text-rgb),0.75)' }}>
                      <p>Approved by: <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{approvalStatus.signed_by}</span></p>
                      <p style={{ color: 'rgba(var(--accent-rgb),0.65)' }}>Date: {new Date(approvalStatus.signed_at).toLocaleString()}</p>
                      {approvalStatus.notes && <p>Notes: {fixName(approvalStatus.notes)}</p>}
                    </div>
                    {actionTickets.length > 0 && (
                      <div className="mt-3 space-y-1">
                        <p className="text-xs font-semibold" style={{ color: 'rgba(var(--accent-rgb),0.8)' }}>Action Tickets:</p>
                        {actionTickets.map(t => (
                          <button key={t.id} onClick={() => setPreviewTicket(t)}
                            ...
                            <span className="flex items-center gap-1.5">
                              <Ticket className="w-3.5 h-3.5" style={{ color: 'var(--accent)' }} />
                              <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{t.ticket_number}</span>
                              <span style={{ color: 'rgba(var(--accent-rgb),0.7)' }}>→ {fixName(t.assigned_to)}</span>
                            </span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(['self', 'assign'] as const).map(mode => (
                        <button key={mode} type="button" onClick={() => setApprovalMode(mode)}
                          ...
                          style={{
                            border: approvalMode === mode ? '1px solid rgba(var(--accent-rgb),0.45)' : '1px solid rgba(var(--accent-rgb),0.18)',
                            background: approvalMode === mode ? 'rgba(var(--accent-rgb),0.2)' : 'rgba(0,0,0,0.15)',
                            color: approvalMode === mode ? 'var(--accent-text)' : 'rgba(var(--accent-text-rgb),0.6)',
                          }}>
                      ...
                      <div className="space-y-2">
                        <div className="relative" ref={dropdownRef}>
                          <button type="button" onClick={() => setDropdownOpen(v => !v)}
                            ...
                            style={{ border: assignTo ? '1px solid rgba(var(--accent-rgb),0.4)' : '1px solid rgba(var(--accent-rgb),0.2)', background: 'var(--input-bg)', color: assignTo ? 'var(--accent-text)' : 'rgba(var(--accent-rgb),0.5)' }}>
                            <span className="truncate">{assignTo || 'Assign to...'}</span>
                            <ChevronDown className="w-4 h-4 flex-shrink-0 ml-2 transition-transform" style={{ color: 'var(--accent)', transform: dropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
                          </button>
                          {dropdownOpen && (
                            <div className="absolute z-20 w-full mt-1 rounded-xl overflow-hidden shadow-lg" style={{ background: 'var(--card-bg)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(var(--accent-rgb),0.2)' }}>
                              {ASSIGNEES.map(name => (
                                <button key={name} type="button" onClick={() => { setAssignTo(name); setDropdownOpen(false); }}
                                  ...
                                  style={{ color: assignTo === name ? 'var(--accent-text)' : 'rgba(var(--accent-text-rgb),0.7)' }}
                                  onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(var(--accent-rgb),0.12)')}
                                  onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
                                  <span>{name}</span>
                                  {assignTo === name && <Check className="w-4 h-4" style={{ color: 'var(--accent)' }} />}
                                </button>
                              ))}
                            </div>
                          )}
                        ...
                        <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)}
                          className="w-full px-3 py-2 text-sm rounded-xl focus:outline-none"
                          style={{ background: 'var(--input-bg)', border: '1px solid rgba(var(--accent-rgb),0.2)', color: 'var(--accent-text)' }} title="Due Date / Deadline" />
                  ...
                  <p className="text-xs italic" style={{ color: 'rgba(var(--accent-rgb),0.45)' }}>Complete Step 1 (For Review) first before proceeding to approval.</p>
                )}
              </div>
            </div>
    ...
    {showReviewConfirm && (
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
        <div className="rounded-2xl p-6 max-w-sm w-full" style={{ background: 'var(--card-bg)', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', border: '1px solid rgba(var(--accent-rgb),0.2)', boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
          <h2 className="text-lg font-bold mb-2" style={{ color: 'var(--accent-text)' }}>Confirm Review</h2>
          <p className="text-sm mb-1" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Reviewer: <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{SIR_LENMARK}</span></p>
          {reviewNotes && <p className="text-sm mb-3" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Notes: {reviewNotes}</p>}
          <p className="text-xs mb-5" style={{ color: 'rgba(var(--accent-rgb),0.6)' }}>This cannot be undone once saved.</p>
          <div className="flex gap-3">
            <button onClick={() => setShowReviewConfirm(false)} className="flex-1 px-4 py-2 rounded-xl text-sm font-medium" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(var(--accent-rgb),0.2)', color: 'rgba(var(--accent-text-rgb),0.7)' }}>Cancel</button>
            <button onClick={handleReviewSubmit} className="flex-1 px-4 py-2 text-white rounded-xl text-sm font-medium" style={{ backgroundColor: 'var(--primary)' }}>Confirm</button>
          </div>
        </div>
      </div>
    )}
    ...
    {showApprovalConfirm && (
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
        <div className="rounded-2xl p-6 max-w-sm w-full" style={{ background: 'var(--card-bg)', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', border: '1px solid rgba(var(--accent-rgb),0.2)', boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
          <h2 className="text-lg font-bold mb-2" style={{ color: 'var(--accent-text)' }}>Confirm Approval</h2>
          <p className="text-sm mb-1" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Approver: <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{SIR_RONALD}</span></p>
          {approvalMode === 'assign' && (
            <>
              <p className="text-sm mb-1" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Assigned to: <span className="font-medium" style={{ color: 'var(--accent-text)' }}>{resolvedAssignTo}</span></p>
              {dueDate && <p className="text-sm mb-1" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Due: {new Date(dueDate).toLocaleDateString()}</p>}
              {instruction && <p className="text-sm mb-1" style={{ color: 'rgba(var(--accent-text-rgb),0.7)' }}>Instruction: {instruction}</p>}
          ...
          <p className="text-xs mb-5" style={{ color: 'rgba(var(--accent-rgb),0.6)' }}>This cannot be undone once saved.</p>
          <div className="flex gap-3">
            <button onClick={() => setShowApprovalConfirm(false)} className="flex-1 px-4 py-2 rounded-xl text-sm font-medium" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(var(--accent-rgb),0.2)', color: 'rgba(var(--accent-text-rgb),0.7)' }}>Cancel</button>
            <button onClick={handleApprovalSubmit} className="flex-1 px-4 py-2 text-white rounded-xl text-sm font-medium" style={{ backgroundColor: 'var(--primary)' }}>Confirm</button>
          </div>
        </div>
      </div>
    )}
