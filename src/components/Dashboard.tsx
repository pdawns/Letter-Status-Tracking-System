  ...
  return (
    <div className="px-4 pt-3 pb-2 flex flex-col gap-3" style={{ height: 'calc(100vh - 140px)', overflow: 'hidden' }}>
      ...
      <div className="flex gap-2 flex-shrink-0">
        {[
          { label: 'Total',     value: letters.length,   sub: `${todayCount} today`,     color: 'var(--accent-text)', bg: 'rgba(var(--accent-text-rgb),0.07)', bdr: 'rgba(var(--accent-text-rgb),0.14)', Icon: FileText },
      ...
      <div className="flex gap-3 flex-1 min-h-0">
                ...
                return (
                  <button key={v} onClick={() => setRecentView(v)}
                    ...
                    style={{
                      ...
                      border: recentView === v ? `1px solid ${color}50` : '1px solid rgba(var(--accent-rgb),0.1)',
                      color: recentView === v ? color : 'rgba(var(--accent-text-rgb),0.35)',
                    }}>
              ...
              return (
                <div key={l.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl"
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold leading-tight" style={{ color: 'var(--accent-text)' }}>{l.reference_number}</p>
                    <p className="text-xs truncate mt-0.5" style={{ color: 'rgba(var(--accent-text-rgb),0.55)' }}>{l.title}</p>
                    {l.document_type && (
                ...
                return (
                  <button key={v} onClick={() => setStatusView(v)}
                    ...
                    style={{
                      ...
                      border: statusView === v ? `1px solid ${color}50` : '1px solid rgba(var(--accent-rgb),0.1)',
                      color: statusView === v ? color : 'rgba(var(--accent-text-rgb),0.35)',
                    }}>
                    {v} ({list.length})
                  </button>
                );
              ...
              return (
                <div key={l.id} className="px-3 py-2.5 rounded-xl"
                  style={{
                    ...
                    border: isOver && statusView === 'pending' ? '1px solid rgba(239,68,68,0.18)' : '1px solid rgba(var(--accent-rgb),0.08)',
                  }}>
        ...
        <div className="flex flex-col rounded-2xl overflow-hidden" style={{ ...glass(), flex: 1, minWidth: 0 }}>
          <div className="flex items-center gap-2 px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid rgba(var(--accent-rgb),0.1)' }}>
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'rgba(var(--accent-rgb),0.7)' }}>Doc Types</span>
          ...
          <div className="flex-1 flex flex-col items-center px-4 py-4 gap-4 overflow-y-auto">
            {typeEntries.length === 0 ? (
              <p className="text-xs text-center py-4" style={{ color: 'rgba(var(--accent-rgb),0.4)' }}>No documents yet</p>
            ) : (() => {
              const total = typeEntries.reduce((s, [, c]) => s + c, 0);
              const size = 220;
              const cx = size / 2, cy = size / 2, r = 96, inner = 58;
              let angle = -Math.PI / 2;
              const slices = typeEntries.map(([type, count], i) => {
                ...
                return { type, count, color, d };
              });
              return (
                <>
                  <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="flex-shrink-0">
                    {slices.map(s => (
                      <path key={s.type} d={s.d} fill={s.color} opacity={0.9} stroke="rgba(0,0,0,0.25)" strokeWidth="2" />
                    ))}
                    <text x={cx} y={cy - 8} textAnchor="middle" dominantBaseline="middle"
                      fill="rgba(var(--accent-text-rgb),0.95)" fontSize="30" fontWeight="bold">{total}</text>
                    <text x={cx} y={cy + 18} textAnchor="middle" dominantBaseline="middle"
                      fill="rgba(var(--accent-rgb),0.6)" fontSize="11">total</text>
                  </svg>
                  <div className="w-full space-y-2.5 pb-2">
                    {slices.map(s => (
                      <div key={s.type} className="flex items-center gap-3 px-2 py-2 rounded-xl"
                        style={{ background: `${s.color}12`, border: `1px solid ${s.color}28` }}>
                        <div className="w-3.5 h-3.5 rounded flex-shrink-0" style={{ background: s.color }} />
                        <span className="text-sm capitalize flex-1 truncate font-medium" style={{ color: 'rgba(var(--accent-text-rgb),0.85)' }}>{s.type}</span>
                        <span className="text-sm font-bold flex-shrink-0" style={{ color: s.color }}>{s.count}</span>
                        <span className="text-xs w-10 text-right flex-shrink-0 font-semibold" style={{ color: 'rgba(var(--accent-rgb),0.6)' }}>
